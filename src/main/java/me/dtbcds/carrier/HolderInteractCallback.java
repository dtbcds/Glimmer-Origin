...
public class HolderInteractCallback {
    ...
    public void throwInteract(PlayerEntity player, World world) {
    	CarrierComponent carrier = Carrier.HOLDER.get(player);
        CarryingData carrying = carrier.getCarryingData();
        if (carrying == null || carrying.getBlockState() == null) return;
    	if(carrying.getBlockState().getBlock() instanceof AbstractChestBlock<?> || carrying.getBlockState().getBlock() instanceof AbstractBannerBlock || carrying.getBlockState().getBlock() == Blocks.SPAWNER) return;
        Carriable<?> carriable = CarriableRegistry.INSTANCE.get(carrying.getType());
    	carriable.tryThrow(carrier, world);
        return;
    }
}
